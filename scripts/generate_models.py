...
**generate** (snapshots -> ``src/ena_api/_generated/``).  Both run by default;
...
#: Where the generated modules are written.  Phase 4 moves this to ``models``,
#: once the handwritten ``models.py`` it would shadow is gone.
OUTPUT: Final = ROOT / "src" / PACKAGE.replace(".", "/") / "_generated"
...
REPORT_MODEL_FIELDS: Final[dict[str, tuple[str, ...]]] = {
    ...
    "files": ("accession", "run_accession", "filename", "checksum", "checksum_method"),
}
...
#: is the drift alarm.  ``/report/files``, which ``list_files()`` calls, is
#: deliberately absent: it is not in ENA's spec (CODEGEN_PLAN.md §0.3.1).
...
def render_report_module(entity: str, fields: list[dict[str, str]]) -> str:
    """The source of one ``_generated/reports/<entity>.py``."""
...
def render_reports_init(entities: list[str]) -> str:
    """The source of ``_generated/reports/__init__.py``."""
...
def render_endpoints() -> str:
    """The source of ``_generated/endpoints.py``, checked against the snapshots.
...
def render_webin() -> str:
    """The source of ``_generated/webin.py``: the async-queue submission job."""
...
def render_init(entities: list[str], xml_entities: tuple[str, ...], tags: tuple[str, ...]) -> str:
    """The source of ``_generated/__init__.py``: the lookup tables."""
    ...
    lines = [
        ...
        *(f"    ReportEntity.{snake(entity).upper()}: {REPORT_ENTITIES[entity]}Report," for entity in entities),
        ...
        "",
        f"__all__ = {sorted([*models, 'REPORT_MODELS', 'RECEIPT_ENTITY_TAGS', 'ReportEntity', 'XML_ENTITIES'])!r}",
    ]
    return "\n".join(lines) + "\n"
